...
static void
sigterm_handlerz(int sig)//zwz
{
	zswitch = zswitch?0:1;
	signal(SIGTSTP , sigterm_handlerz);//zwz
}
*/
...
/*
	av_log(NULL, AV_LOG_INFO, "gotframe %d, format is %d, width %d, height %d, %d, %d\n", 
		*got_frame, frame->format, frame->width, frame->height, 
		frame->linesize[0], frame->linesize[1]);//zwz
*/
	...
	if(zswitch){
		if(*got_frame){
			int zi;
			for(zi=0; zi<frame->width * frame->height * 2; zi++){
				if(zi%2 == 1){
					frame->data[0][zi] = 128;
				}
			}
			av_log(NULL, AV_LOG_INFO, "processed one frame\n");
		}
	}
	*/
/*
	char outname[128];//zwz
	memset(outname, 0 ,128);
	sprintf(outname, "/home/zwz/pptest/%d.jpg", zindex++);
	AVFrame *zframe = av_frame_alloc();
	av_frame_copy(zframe, frame);
	saveJpg(zframe, outname); 
	av_frame_free(&zframe);
	*/
...
static int send_frame_to_filters(InputStream *ist, AVFrame *decoded_frame)
{
	...
	//av_log(NULL, AV_LOG_INFO, "filters %d", ist->nb_filters);//zwz
