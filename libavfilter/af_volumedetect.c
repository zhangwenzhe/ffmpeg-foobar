...
static int filter_frame(AVFilterLink *inlink, AVFrame *samples)
{
    ...
    for (plane = 0; plane < nb_planes; plane++) {
        ...
        for (i = 0; i < nb_samples; i++){
			if(pcm[i] + 0x8000 > max_volume){
				max_volume = pcm[i] + 0x8000;
			}
	...
	if(max_volume > maxall){
		maxall = max_volume;
	}
	
	//av_log(ctx, AV_LOG_INFO, "max_volume value: %d\n", max_volume);
    //av_log(ctx, AV_LOG_INFO, "max_volume: %.1f dB\n", -logdb(max_volume * max_volume));

	if(max_volume >= 62000){
		system("/home/zwz/ffmpeg-4.1.3/trigger.sh &");
	}
...
{
	av_log(ctx, AV_LOG_INFO, "maxall value: %d\n", maxall);
    //print_stats(ctx);
}
